...
list utility_list = []; // Line 11 - List of theta values for every player [0.33,0.33,0.33] standard
...
//Market Variables
list inventory_list = []; // 2D pseudo array for player inventories. For players 0 to n, where 0 is the governent, and n is the total nubmer of players. Player n quantitiy good 1, player n quantitiy good 2, player n quantitiy good 3, player n quantity tokens, player n utility,

list harvest_training = [];
integer harvest_handle = FALSE;
list bid_training = [];
integer bid_handle = FALSE;
list ask_training = [];
integer ask_handle = FALSE;
...
{
    if(experiment_state == 0) // Experiment set to "off" or "training"
    {
        running = FALSE;
        treatment = FALSE;
        hud_message(-4,0);
...
training_toggle(integer training_state) // Turns the experiment on and off.
{
    if(training_state == 1)
    {
        training = TRUE;
        ...
        announce_market();
        generate();
        llSay(0,"Training Start.");
        hud_message(-11,0);
    }
    else if(training_state == 0)
    {
        training = FALSE;
        hud_message(-4,0);
        llSay(0,"Training End.");
    }
}
...
harvest_trainer(list harvest_check)
{
    integer training_check = llList2Integer(harvest_check,0) - 1;
    harvest_training = llListReplaceList(harvest_training, (list)1, training_check, training_check);
    llSay(0,llList2CSV(harvest_training));
    generate();
    if(llListFindList(harvest_training, (list)0) == -1)
    {
        harvest_handle = FALSE;
        hud_message(-12,0);
        bid_handle = TRUE;
    }
}

bid_trainer(list bid_check)
{
    integer training_check = llList2Integer(bid_check,0) - 1;
    if(llList2Integer(bid_check,4) == 2) // A bid of 10 for cubes.
    {
        bid_training = llListReplaceList(bid_training, (list)1, training_check, training_check);
        llSay(0,llList2CSV(bid_training));
        if(llListFindList(bid_training, (list)0) == -1)
        {
            bid_handle = FALSE;
            hud_message(-13,0);
            ask_handle = TRUE;
...
ask_trainer(list ask_check)
{
    integer training_check = llList2Integer(ask_check,0) - 1;
    if(llList2Integer(ask_check,4) == 3 && llList2Integer(ask_check,5) == 10) // An ask of 10 for rings.
    {
        bid_training = llListReplaceList(ask_training, (list)1, training_check, training_check);
        llSay(0,llList2CSV(ask_training));
        if(llListFindList(ask_training, (list)0) == -1)
        {
            ask_handle = FALSE;
            training_toggle(0);
        }
...
announce_market()
{
    market_list = [standing_bid_good_1,standing_bidder_good_1,standing_ask_good_1,standing_asker_good_1,standing_bid_good_2,standing_bidder_good_2,standing_ask_good_2,standing_asker_good_2,standing_bid_good_3,standing_bidder_good_3,standing_ask_good_3,standing_asker_good_3];
...
{
    //llSay(0, "Gov buy");
    float decision = llFrand(1.0); // Random float to determine buy
    if(decision < govt_buy_prob) // Determines whether or not to buy.
    {
        gov_quantity_good_1 = llList2Integer(inventory_list, get_index(0, GOOD_1_INDEX));
        gov_quantity_good_2 = llList2Integer(inventory_list, get_index(0, GOOD_2_INDEX));
        gov_quantity_good_3 = llList2Integer(inventory_list, get_index(0, GOOD_3_INDEX));
        if(gov_quantity_good_1 < gov_quantity_good_2 && gov_quantity_good_1 < gov_quantity_good_3 && standing_ask_good_1 > 0 && standing_ask_good_1 < govt_buy_range * max_price)// Has least of good 1 and it is offered for sale and current ask is less than 125% of last contract price
        {
            standing_bid_good_1 = standing_ask_good_1; // sets gov't bid to standing ask
            standing_bidder_good_1 = 0;
            print_money(standing_bid_good_1);
            string rec = "0,0.0,0.0,0.0,1," + (string)standing_bid_good_1;
            record(3, rec); // Bid record: Player, Position X, Position Y, Position Z, Good Number, Bid Price.
            contract(1, 0, standing_asker_good_1, standing_bid_good_1, standing_ask_good_1);
        }
        else if(gov_quantity_good_2 < gov_quantity_good_1 && gov_quantity_good_2 < gov_quantity_good_3 && standing_ask_good_2 > 0 && standing_ask_good_2 < govt_buy_range * max_price)// Has least of good 1 and it is offered for sale and current ask is less than 125% of last contract price
        {
            standing_bid_good_2 = standing_ask_good_2; // sets gov't bid to standing ask
            standing_bidder_good_2 = 0;
            print_money(standing_bid_good_2);
            string rec = "0,0.0,0.0,0.0,2," + (string)standing_bid_good_2;
            record(3, rec); // Bid record: Player, Position X, Position Y, Position Z, Good Number, Bid Price.
            contract(2, 0, standing_asker_good_2, standing_bid_good_2, standing_ask_good_2);
        }
        else if(gov_quantity_good_3 < gov_quantity_good_1 && gov_quantity_good_3 < gov_quantity_good_2 && standing_ask_good_3 > 0 && standing_ask_good_3 < govt_buy_range * max_price)// Has least of good 1 and it is offered for sale and current ask is less than 125% of last contract price
        {
            standing_bid_good_3 = standing_ask_good_3; // sets gov't bid to standing ask
            standing_bidder_good_3 = 0;
            print_money(standing_bid_good_3);
            string rec = "0,0.0,0.0,0.0,3," + (string)standing_bid_good_3;
            record(3, rec); // Bid record: Player, Position X, Position Y, Position Z, Good Number, Bid Price.
            contract(3, 0, standing_asker_good_3, standing_bid_good_3, standing_ask_good_3);
        }
        else // Tie break
        {
            float roll_dice = llFrand(3.0);
            if(roll_dice < 1.0 && standing_ask_good_1 > 0 && standing_ask_good_1 < 1.25 * max_price) //Buy Good 1
            {
                standing_bid_good_1 = standing_ask_good_1; // sets gov't bid to standing ask
                standing_bidder_good_1 = 0;
                print_money(standing_bid_good_1);
            ...
            else if(roll_dice > 1.0 && roll_dice < 2.0 && standing_ask_good_2 > 0 && standing_ask_good_2 <  1.25 * max_price)
            {
                standing_bid_good_2 = standing_ask_good_2; // sets gov't bid to standing ask
                standing_bidder_good_2 = 0;
                print_money(standing_bid_good_2);
            ...
            else if(roll_dice > 2.0 && standing_ask_good_3 > 0 && standing_ask_good_3 < 1.25 * max_price)
            {
                standing_bid_good_3 = standing_ask_good_3; // sets gov't bid to standing ask
                standing_bidder_good_3 = 0;
                print_money(standing_bid_good_3);
...
print_money(integer token_amount)
{
    //llSay(0,"Print Money");
...
state training
{
    ...
    touch_start(integer number)
    {
        if(llDetectedKey(0) == admin && training == TRUE)
        {
            llDialog (admin, "Return to setup?", ["Yes","No"], EXP_CONTROL);
        }
        else if(llDetectedKey(0) != admin) // admin verification
        {
            integer user = llListFindList(ADMIN_LIST, llKey2Name(llDetectedKey(0))); // Verifies that the user has been granted admin status, by searching the master "admin" list.
    ...
    listen(integer channel, string name, key id, string message)
    {
        if(channel == RECEIVER)
        {
            ...
            // 1 - Bid training
            // 2 - Ask training
            if(message_type == 0 && harvest_handle == TRUE) // Harvest
            {
                harvest_trainer(temporary_list);
            }
            else if(message_type == 1 && bid_handle == TRUE) // Bid
            {
                bid_trainer(temporary_list);
            }
            else if(message_type == 2 && ask_handle == TRUE) // Ask
            {
                ask_trainer(temporary_list);
            }
        }
        else if(channel == EXP_CONTROL)
        {
            if(message == "Yes") // Experiment begins
            {
                state default;
            }
        }
